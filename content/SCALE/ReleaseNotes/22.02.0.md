...
This mockup explores the idea of combining the existing 11 SCALE Angelfish release notes into a single article.
If adopted, this single article becomes the location for all subsequent release notes of the major version.

Anticipated benefits:
- Drastically reduced URL creation and user confusion when searching for release notes
- Eliminate redundant text and reduced possibility of text drift errors from similar copy/pasted sections.
- Collect all changes for a major version, including prerelease and postrelease, into a single easy-to-search article.
- Allows communicating major features about a release in a way that gets seen or reinforced with every update.

The info boxes point out various places for efficiencies or improvements over the current strategy.

{{< hint info >}}
Look through the Table of Contents and see all the duplicated headers.
These can all be combined into single sections and greatly reduce both the redundant language but also prevent text or confusion errors from one minor release to the next.
{{< /hint >}}
...
{{< hint info >}}
A single high-level writeup of the expected major software features can be provided.
It becomes "living" text that can be altered as development plans or feature building changes throughout the prerelease cycles.
{{< /hint >}}

iXsystems is pleased to announce the next member of the TrueNAS family: TrueNAS SCALE!
SCALE has all major TrueNAS CORE storage and sharing features and web interface based on Debian GNU/Linux.
There are also be additional SCALE-specific features that are derived from the application’s Linux base.
...
The first major release of TrueNAS SCALE is code-named Angelfish and follows a *year.month-NAME.#* versioning scheme.

{{< hint info >}}
Each new minor release can be "stacked" on top of the previous release.
While the raw text of the article becomes lengthy, the most current information remains at the top of the article and older releases notes are immediately accessible (or linkable) using the article table of contents.
{{< /hint >}}
...
{{< hint info >}}
Every minor release now can minimize the redundant text and features description and instead focus on the factual changelog and any important notes that are specific to only that release.
{{< /hint >}}
...
This is the first public prerelease version of TrueNAS SCALE Angelfish.
